...
public class EaseChatFragment extends EaseBaseFragment implements EMMessageListener {
    ...
    private String[] permissions4 = {Manifest.permission.ACCESS_FINE_LOCATION};
    ...
    protected TextView group_tv;
    protected TextView group_name;
    protected Button group_button;
    protected FrameLayout frameLayout;
    protected TextView tv1_yuan;
    protected TextView tv2_yuan;
    protected EditText et_yuan;
    protected Button button_yuan;
    protected RelativeLayout hundred_rl;
    protected Button hundred_bt;
    protected TextView hundred_tv2;
    protected String hundred_id;
    protected String groupName;
    protected EaseChatExtendMenu match_input;
    protected RadioGroup group1;
    protected RadioGroup group2;
    protected RadioGroup group3;
    protected RadioGroup group4;
    protected String socor;
    ...
    public void setVisibility(int i) {
        ...
        chatlinearLayout.setVisibility(i);
    }
    ...
    protected void initView() {
        chatlinearLayout = (LinearLayout) getView().findViewById(R.id.chatlineartlayout);
        match_input = (EaseChatExtendMenu) getView().findViewById(R.id.match_input);
        tv_step = (TextView) getView().findViewById(R.id.tv_step);
        tv_deteil = (TextView) getView().findViewById(R.id.tv_detail);
        ...
        group_button = (Button) getView().findViewById(R.id.group_button);
        frameLayout = (FrameLayout) getView().findViewById(R.id.chat_view);
        tv1_yuan = (TextView) getView().findViewById(R.id.tv1_yuan);
        tv2_yuan = (TextView) getView().findViewById(R.id.tv2_yuan);
        et_yuan = (EditText) getView().findViewById(R.id.et_yuan);
        button_yuan = (Button) getView().findViewById(R.id.button_yuan);
        hundred_rl = (RelativeLayout) getView().findViewById(R.id.hundred_rl);
        hundred_bt = (Button) getView().findViewById(R.id.hundred_bt);
        hundred_tv2= (TextView) getView().findViewById(R.id.hundred_tv2);
        group1 = (RadioGroup) getView().findViewById(R.id.socore_radiogroup);
        group2 = (RadioGroup) getView().findViewById(R.id.car_radiogroup1);
...
//        messageList.setAvatarShape(1);
        ...
        inputMenu.setChatInputMenuListener(new ChatInputMenuListener() {
            ...
            public boolean onPressToSpeakBtnTouch(View v, MotionEvent event) {
                ...
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    // 检查该权限是否已经获取
                    str="录音";
                    int j = ContextCompat.checkSelfPermission(getContext(), permissions2[0]);
    ...
    protected void setUpView() {
        ...
        titleBar.setBackgroundColor(Color.parseColor("#f76243"));
        if (chatType == EaseConstant.CHATTYPE_SINGLE) {
            // set title
            ...
            titleBar.setRightImageResource(R.drawable.menu);
        } else {
                ...
                getActivity().runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        if (room != null) {
                            ...
                            try {
                                titleBar.setTitle(EMClient.getInstance().chatroomManager().fetchChatRoomFromServer(toChatUsername).getName());
                            } catch (HyphenateException e) {
                                e.printStackTrace();
                            }
                        }
            ...
            switch (itemId) {
                case ITEM_TAKE_PICTURE:
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        // 检查该权限是否已经获取
                        str="相机";
                        int j = ContextCompat.checkSelfPermission(getContext(), permissions1[0]);
                        // 权限是否已经 授权 GRANTED---授权  DINIED---拒绝
                        if (j != PackageManager.PERMISSION_GRANTED) {
                            // 如果没有授予该权限，就去提示用户请求
                            // showDialogTipUserRequestPermission();
                            startRequestPermission(permissions1);
                        }else {
                            selectPicFromCamera();
                        }
                    }else {
                ...
                case ITEM_LOCATION:
                    ...
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                        str="位置";
                        // 检查该权限是否已经获取
                        int j = ContextCompat.checkSelfPermission(getContext(), permissions4[0]);
                        // 权限是否已经 授权 GRANTED---授权  DINIED---拒绝
                        if (j != PackageManager.PERMISSION_GRANTED) {
                            // 如果没有授予该权限，就去提示用户请求
                            // showDialogTipUserRequestPermission();
                            startRequestPermission(permissions4);
                        }else {
                            startActivityForResult(new Intent(getContext(), EaseBaiduMapActivity.class), REQUEST_CODE_MAP);
                        }
                    }else {
                        startActivityForResult(new Intent(getContext(), EaseBaiduMapActivity.class), REQUEST_CODE_MAP);
                    }
...
//621790  010001 3050227
            ...
            if (requestCode == 321) {
                if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                    ...
                    if (grantResults[0] != PackageManager.PERMISSION_GRANTED) {
                        ...
                        Log.e("判断用户是否",permissions[0]);
                        boolean b = shouldShowRequestPermissionRationale(permissions[0]);
                        if (!b) {
                            Log.e("提示用户去应用设置界面手动开启权限",permissions[0]);
                            // 用户还是想用我的 APP 的
                            // 提示用户去应用设置界面手动开启权限
                            showDialogTipUserGoToAppSettting();
                        }
                    } else {
                        Log.e("继续",permissions[0]);
                        if (str.equals("相机")){
                            selectPicFromCamera();
                        }
                        if (str.equals("拍照")){
                            chatFragmentHelper.takePhoto();
                        }
                        if (str.equals("定位")){
                            startActivityForResult(new Intent(getContext(), EaseBaiduMapActivity.class), REQUEST_CODE_MAP);
                        }
    ...
    private void showDialogTipUserGoToAppSettting() {
        ...
        alertDialog = new AlertDialog.Builder(getContext())
                .setTitle(str+"权限不可用")
                .setMessage("请在-应用设置-权限-中，打开"+ str+"权限")
    ...
    public void setMatch_input(){
        match_input.setVisibility(View.VISIBLE);
        inputMenu.setVisibility(View.GONE);
    }
    public void setNormal_input(){
        match_input.setVisibility(View.GONE);
