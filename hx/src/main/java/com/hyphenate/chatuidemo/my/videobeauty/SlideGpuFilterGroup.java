...
public class SlideGpuFilterGroup {
    ...
    public SlideGpuFilterGroup() {
        initFilter();
        scroller = new Scroller(BaseApplication.getContext());
    }
    ...
    private GPUImageFilter getFilter(int index) {
        MagicBeautyFilter beautyFilter=new MagicBeautyFilter();
        beautyFilter.setBeautyLevel(5);
        GPUImageFilter filter = beautyFilter;
        if (filter == null) {
    ...
    public void onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                locked = true;
                downX = -1;
                if (offset > BaseApplication.screenWidth / 3) {
                    scroller.startScroll(offset, 0, BaseApplication.screenWidth - offset, 0, 100 * (1 - offset / BaseApplication.screenWidth));
                    needSwitch = true;
                } else {
                    scroller.startScroll(offset, 0, -offset, 0, 100 * (offset / BaseApplication.screenWidth));
                    needSwitch = false;
                }
                break;
        }
