...
import static com.hyphenate.chatuidemo.my.videobeauty.BeautyCamerView.BUTTON_STATE_BOTH;
import static com.hyphenate.chatuidemo.my.videobeauty.BeautyCamerView.BUTTON_STATE_ONLY_CAPTURE;
import static com.hyphenate.chatuidemo.my.videobeauty.BeautyCamerView.BUTTON_STATE_ONLY_RECORDER;
...
public class CaptureButton extends View {
    ...
    public CaptureButton(Context context, int size) {
        ...
        button_state = BUTTON_STATE_ONLY_RECORDER;  //初始化按钮为可录制不可拍照

        duration = 30 * 1000;              //默认最长录制时间为30s

        min_duration = 2*1000;              //默认最短录制时间为1.5s
        ...
        switch (event.getAction()) {
            case MotionEvent.ACTION_DOWN:
                ...
                if ((button_state == BUTTON_STATE_ONLY_RECORDER || button_state == BUTTON_STATE_BOTH))
                    postDelayed(longPressRunnable, 1);    //同时延长500启动长按后处理的逻辑Runnable
                break;
