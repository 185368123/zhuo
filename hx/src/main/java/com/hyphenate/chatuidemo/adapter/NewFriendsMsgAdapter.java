...
public class NewFriendsMsgAdapter extends ArrayAdapter<InviteMessage> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (msg != null) {
            ...
            UserDB user=UserMsgDBHelp.getUserMsgDBHelp().searchByUserId(msg.getFrom());
            if (user!=null){
                Glide.with(context).load(user.getPhoto_link()).into(holder.avator);
                holder.name.setText(user.getNick_name());
            }
			if (msg.getStatus() == InviteMessageStatus.BEAGREED) {
                holder.agreeBtn.setVisibility(View.GONE);
				holder.refuseBtn.setVisibility(View.GONE);
                DemoHelper.getInstance().getContactList().put(msg.getFrom(),new EaseUser(msg.getFrom()));
                ...
                switch (status) {
                    case AGREED:
                        ...
                        break;
                    case REFUSED:
                        List<UserDB> userDBList=  DataSupport.select().where("user_id = ?", msg.getFrom()).find(UserDB.class);
                        if (!(userDBList.size()>0)){
                            ...
                            getUserMsgPresenter.getUserMsg(msg.getFrom());
                        }else {
                            Glide.with(context).load(userDBList.get(0).getPhoto_link()).into(holder.avator);
                            holder.name.setText(userDBList.get(0).getNick_name());
                        }
